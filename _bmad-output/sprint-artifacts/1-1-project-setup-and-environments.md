...
1. Local bootstrap succeeds: after `npm install` and `npm run dev`, the app starts without blocking errors and a `/api/health` endpoint responds 200 within 500ms against the dev database. (Source: \_bmad-output/epics.md#Epic-1-Foundation--Access)
2. `.env.example` lists all required secrets/config (auth provider, database URL, NextAuth session settings, telemetry toggles) with no secrets committed; missing required envs fail fast with clear errors. (Source: \_bmad-output/prd.md#Functional-Requirements)
3. Prisma connectivity validated: migrations run against the configured database; health check verifies DB connectivity and reports status. (Source: \_bmad-output/architecture.md#Executive-Summary)
4. Deployment baseline: Vercel project + Neon Postgres configured with required env vars; deployed health endpoint responds 200 within 500ms. (Source: \_bmad-output/epics.md#Epic-1-Foundation--Access)
5. Build pipeline or Vercel checks complete successfully (lint/tests/build) with no manual fixes; setup steps are documented for future stories. (Source: \_bmad-output/architecture.md#Project-Initialization)
...
- [x] Documentation (AC: #2, #5)
  - [x] Add or update a short README section for setup/run/deploy steps and where story artifacts live (`_bmad-output/sprint-artifacts`).
...
- Architecture alignment: Next.js 16 + React 19 with hybrid GraphQL/REST; use REST for `/api/health` and verify Prisma Postgres connectivity before responding. (Source: \_bmad-output/architecture.md#Executive-Summary)
- Auth/RBAC posture: sessions via NextAuth with admin/manager/viewer roles; ensure env vars cover session secrets and provider settings even if provider is stubbed for now. (Source: \_bmad-output/prd.md#Roles--Access)
- Operational guardrails: keep secrets out of repo, rely on Vercel/Neon env storage, and log requestId/user context on health check for tracing. (Source: \_bmad-output/architecture.md#Security-Architecture)
- Multi-league/season scope: seed and migrations should include league/season IDs where applicable to avoid future refactors; verify schema alignment before first deploy. (Source: \_bmad-output/prd.md#Product-Scope)

### Project Structure Notes

- Use `_bmad-output/sprint-artifacts` for story outputs; app source follows feature-first layout per architecture doc.
...
- [Source: _bmad-output/epics.md#Epic-1-Foundation--Access]
- [Source: _bmad-output/prd.md#Functional-Requirements]
- [Source: _bmad-output/prd.md#Roles--Access]
- [Source: _bmad-output/prd.md#Product-Scope]
- [Source: _bmad-output/architecture.md#Executive-Summary]
...
- \_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.context.xml
...
- \_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md

## Change Log

- Draft created for Story 1-1-project-setup-and-environments.
- Ready-for-dev with context generated (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.context.xml).
...
- High: Deployment baseline tasks unchecked; no evidence of Vercel/Neon deploy or 500ms health response in prod (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:27-29).
...
| AC                                          | Status  | Evidence                                                                                                                                                                                                                       |
| ------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1. Dev bootstrap + `/api/health` 200 <500ms | PARTIAL | Health endpoint exists with DB probe but no build metadata/logging and no recorded run against dev DB (src/app/api/health/route.ts:8-49; tasks at \_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:23-24). |
...
| 4. Deployment baseline (Vercel + Neon)      | MISSING | Deployment tasks unchecked; no deploy evidence (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:27-29).                                                                                                   |
...
- [ ] [High] Complete deployment baseline: configure Vercel + Neon env vars, deploy, and verify `/api/health` returns 200 <500ms; document result in story/change log (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md).
...
- Health endpoint now returns build metadata, logs timing, and verifies DB connectivity; production deploy responds 200 with 270ms latency and commit `9b841e5` (src/app/api/health/route.ts:7-68; test/app/api/health/route.test.ts:33-68; \_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:80-84).
...
| 3. Prisma connectivity validated                    | IMPLEMENTED | Health handler executes Prisma `$queryRaw` probe (src/app/api/health/route.ts:25-60) and succeeds in production health check (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:80-84).                                                                                          |
| 4. Deployment baseline (Vercel + Neon)              | IMPLEMENTED | Production health endpoint returns 200/270ms with DB ok (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:80-84).                                                                                                                                                               |
| 5. Build pipeline/Vercel checks succeed             | IMPLEMENTED | `npm run lint` (2025-12-04) and `npm test -- --runInBand` pass locally (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:69-72).                                                                                                                                                |
...
| Deployment baseline (Vercel + Neon)            | [x]       | VERIFIED    | Prod health 200/270ms with DB ok (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:80-84)                                                                    |
| Build/test pipeline                            | [x]       | VERIFIED    | `npm run lint`; `npm test -- --runInBand` passing (\_bmad-output/sprint-artifacts/1-1-project-setup-and-environments.md:69-72)                                                   |
